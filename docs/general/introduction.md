...
- `takeLatest` is used for listening for a particular action. In this case, it will wait for a `LOAD_SENSORS` action. Whenever you disptach this action, the saga will understand that you want to fetch repos from github's public API by calling `getRepos()`.
